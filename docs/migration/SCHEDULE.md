...
| 3 | **folio-mapper** | ▶️ next | The canonical scorer + 4-stage pipeline + FAISS index (the donor of most of this library) | Collapse the Python backend into a thin layer over `folio-resolve`; the repo becomes primarily its TS UI. Behavior-parity tests guard the swap | Real |
